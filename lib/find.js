...
function getNPI(perfObj, callback) {
    ...
    var bloomRes = [];
    var searchOptions = {};
    if (perfObj.offset !== null) {
        searchOptions.offset = perfObj.offset;
    }
    if (perfObj.limit !== null) {
        searchOptions.limit = perfObj.limit;
    }
    buildSearchArray(perfObj, function(searchArr) {
        async.eachSeries(searchArr, function(searchObj, cb) {
            if (check) {
                ...
                bloomClient.search('usgov.hhs.npi', searchObj, searchOptions, function(error, response) {
