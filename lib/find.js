...
function getNPI(perfObj, callback) {
    ...
    buildSearchArray(perfObj, function(searchArr) {
        async.each(searchArr, function(searchObj, cb) {
            ...
            if (bloomRes.length > 0) {
                if (bloomRes.length == 1) {
                    //found only one match, yay
                    callback(null, perfObj, bloomRes[0]); //probably merge the data into original object... for now return result
                } else {
                    //multiple results found
                    callback(null, perfObj, bloomRes);
                }
            } else {
